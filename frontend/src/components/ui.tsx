...
export function Modal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 animate-fadeIn">
      ...
      <div className={`relative w-full ${maxW} bg-white dark:bg-ink-950 rounded-2xl shadow-2xl border border-ink-100 dark:border-ink-800/80 animate-slideUp`}>
        <div className="flex items-center justify-between px-6 py-4 border-b border-ink-100 dark:border-ink-800/80">
          <h2 className="text-base font-semibold text-ink-900 dark:text-white">{title}</h2>
          <button onClick={onClose} className="btn-ghost !px-2 !py-1.5 text-ink-400 hover:text-ink-700 dark:hover:text-white">
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M18 6 6 18M6 6l12 12" /></svg>
          </button>
        </div>
        <div className="px-6 py-5 max-h-[70vh] overflow-y-auto">{children}</div>
        {footer && <div className="px-6 py-4 border-t border-ink-100 dark:border-ink-800/80 flex justify-end gap-2 bg-ink-50/50 dark:bg-ink-900/30 rounded-b-2xl">{footer}</div>}
      </div>
    </div>
  );
}
